...
## 提案資料オートメーション(`/proposal`)

アポイント(商談)のヒアリング内容を入力すると、`lib/courses.ts` のプランと
突き合わせて提案資料を自動生成する内部ツール。

- `/proposal` … ヒアリング内容の入力フォーム(顧客名・業種・予算感・課題など)
- `/proposal/result` … 生成された提案書を表示(印刷 / PDF保存に対応)
- ロジック本体は `lib/proposal.ts`(`generateProposal`)。予算感とプラン価格の
  マッチング、要望テキストの箇条書き化などをルールベースで行う
- お客様向けページではないため、Header のナビゲーションには載せていない
  (事業者側が商談後に URL を直接開いて使う想定)
