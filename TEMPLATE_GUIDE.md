...
`app/proposal/page.tsx`(`/proposal`)で使える、営業担当向けの社内ツール。
アポイント・商談で聞いた内容(課題・予算感・希望時期など)をフォームに入力すると、
`lib/courses.ts` のプランとキーワード一致でマッチングし、提案書ドキュメントのたたき台を自動生成する。

- ロジック本体は `lib/proposal.ts`(`generateProposal` / `matchCourses`)
- 外部 AI API には依存しないルールベース生成のため、生成後は必ず人が内容を確認・調整してから使う
- 生成結果はブラウザの「印刷 → PDF保存」、または「テキストで保存」でファイル出力できる
- サイト訪問者向けのページではないため、Header のナビゲーションには載せていない(`/proposal` に直接アクセスして使う)
